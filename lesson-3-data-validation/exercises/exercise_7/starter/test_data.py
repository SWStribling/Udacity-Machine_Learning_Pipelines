...
def test_class_names(data):
    ...
    assert data['genre'].isin(known_classes).all(), "Column 'genre' has unknown class names"
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        ...
        assert data[col_name].dropna().between(minimum, maximum).all(), (
            f"Column {col_name} has values outside "
            f"the range [{minimum}, {maximum}] which is outside "
        )
